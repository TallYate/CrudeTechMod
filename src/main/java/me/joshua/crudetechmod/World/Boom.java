...
public class Boom {
	...
	@OnlyIn(Dist.CLIENT)
	public Boom(World worldIn, @Nullable Entity entityIn, double x, double y, double z, float size,
			List<BlockPos> affectedPositions) {
		this(worldIn, entityIn, x, y, z, size, false, Explosion.Mode.DESTROY, affectedPositions);
	}

	@OnlyIn(Dist.CLIENT)
	public Boom(World worldIn, @Nullable Entity exploderIn, double xIn, double yIn, double zIn, float sizeIn,
			boolean causesFireIn, Explosion.Mode modeIn, List<BlockPos> affectedBlockPositionsIn) {
	...
	public Boom(World worldIn, @Nullable Entity exploderIn, double xIn, double yIn, double zIn, float sizeIn,
			boolean causesFireIn, Explosion.Mode modeIn) {
		this.world = worldIn;
		...
		this.explosion = new Explosion(this.world, this.exploder, this.x, this.y, this.z, this.size, this.causesFire,
				this.mode);

		this.damageSource = DamageSource.causeExplosionDamage(this.explosion);
				...
				for (float f1 = 0.0F; f1 <= 1.0F; f1 = (float) ((double) f1 + d1)) {
					for (float f2 = 0.0F; f2 <= 1.0F; f2 = (float) ((double) f2 + d2)) {
						...
						if (p_222259_1_.world
								.rayTraceBlocks(new RayTraceContext(vec3d, p_222259_0_,
										RayTraceContext.BlockMode.OUTLINE, RayTraceContext.FluidMode.NONE, p_222259_1_))
					...
					if (j == 0 || j == 15 || k == 0 || k == 15 || l == 0 || l == 15) {
						...
						while (f > 0.0F) {
							...
							if (!blockstate.isAir(this.world, blockpos) || !ifluidstate.isEmpty()) {
								float f2 = Math.max(
										blockstate.getExplosionResistance(this.world, blockpos, exploder,
												this.explosion),
										ifluidstate.getExplosionResistance(this.world, blockpos, exploder,
												this.explosion));
								if (this.exploder != null) {
									f2 = this.exploder.getExplosionResistance(this.explosion, this.world, blockpos,
											blockstate, ifluidstate, f2);
								}
							...
							if (f > 0.0F && (this.exploder == null || this.exploder
									.canExplosionDestroyBlock(this.explosion, this.world, blockpos, blockstate, f))) {
								set.add(blockpos);
							}
		...
		if (flag) {
			...
			for (BlockPos blockpos : this.affectedBlockPositions) {
				...
				if (!blockstate.isAir(this.world, blockpos)) {
					...
					if (blockstate.canDropFromExplosion(this.world, blockpos, this.explosion)
	...
	public LivingEntity getExplosivePlacedBy() {
		if (this.exploder == null) {
			...
			return this.exploder instanceof DamagingProjectileEntity
					? ((DamagingProjectileEntity) this.exploder).shootingEntity
					: null;
		}
	}
